...
function scriptedApi(overrides: {
  ...
  return {
    ...
    workspace: {
      list: r => ok(r, { items: [], archivedSessionIds: [], trashedSessions: [] }),
      ...
      trashSession: r => ok(r, { trashedSessions: [{ sessionId: r.payload.sessionId, deletedAt: 0 }] }),
      restoreSession: r => ok(r, { trashedSessions: [] }),
      deleteTrashedSession: r => ok(r, { trashedSessions: [] }),
    },
...
describe('workspace domain round trip', () => {
  it('routes both workspace methods through their handler rows and value schemas', async () => {
    ...
    expect(list.result).toEqual({ ok: true, value: { items: [], archivedSessionIds: [], trashedSessions: [] } })
