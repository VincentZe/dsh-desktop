...
describe('session export compression config', () => {
  it('defaults to level 6 and rejects values outside the integer 0-9 range', () => {
    expect(ApiProxyService.Config({})).toEqual({
      ...
      externalSessionPollMs: 500,
    })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 0 }))
      .toEqual({ sessionExportCompressionLevel: 0, coldBlankProbeMaxBytes: 1024, externalSessionPollMs: 500 })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 9 }))
      .toEqual({ sessionExportCompressionLevel: 9, coldBlankProbeMaxBytes: 1024, externalSessionPollMs: 500 })
...
describe('cold blank probe config', () => {
  it('accepts a per-Session byte bound including zero and rejects invalid bounds', () => {
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 0 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 0, externalSessionPollMs: 500 })
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 2048 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 2048, externalSessionPollMs: 500 })
...
describe('external session observation config', () => {
  it('accepts positive integer poll intervals and rejects invalid values', () => {
    expect(ApiProxyService.Config({ externalSessionPollMs: 1 })).toEqual({
      sessionExportCompressionLevel: 6,
      coldBlankProbeMaxBytes: 1024,
      externalSessionPollMs: 1,
    })
    for (const value of [0, -1, 1.5]) {
      expect(() => ApiProxyService.Config({ externalSessionPollMs: value })).toThrow()
    }
  })
})
