...
另一个 DSH 进程写入的外部 JSONL 会话（例如 `scripts/dsh-subagent-runner.ts`）在与此 host 共享 persistence 根目录时以只读方式可见。只要 mux 或 host 流保持连接，网关就轮询 snapshots 和 `readFrom()`；第一次观察只建立历史基线，之后向客户端发送 `host/session-added` 和追加的 `session/event` 帧。它不会 attach、resume，也不会把外部会话标记为 running。`externalSessionPollMs` 控制轮询间隔，默认 500 ms；没有 mux 或 host 流时观察器会停止。
