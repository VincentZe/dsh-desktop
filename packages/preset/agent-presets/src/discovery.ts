...
export async function scanRoot(root: PresetRoot): Promise<AgentPreset[]> {
  ...
  let children
  try {
    // Read names instead of relying on Dirent methods. Some fixed-runtime
    // loaders expose the returned entries after serialization, which leaves
    // the name but not `isDirectory()` on each entry.
    children = await readdir(dir, { encoding: 'utf8' })
  } catch (error) {
