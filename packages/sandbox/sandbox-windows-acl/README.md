...
The fixed Web SEA executable uses the same runner contract through an explicit self-dispatch marker: `dsh-web.exe --dsh-windows-acl-runner --workspace ... -- <argv...>`. Node SEA does not execute a JavaScript file supplied after its executable path, so `dsh-sandbox-local` selects this marker when it runs inside the fixed Web runtime. The Web entry consumes the marker before starting the Web profile and invokes the runner in-process; source launches continue to use `runner.js` directly.
