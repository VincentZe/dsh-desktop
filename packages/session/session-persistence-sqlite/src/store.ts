...
export class SqliteStore implements PersistenceBackend<number> {
  ...
  /**
   * Permanently remove one stored session and all of its events. The events
   * table cascades on the session row's foreign key, so one delete reaps the
   * whole durable log.
   * @param id - the session to remove.
   * @param signal - optional cancellation before or after the delete.
   * @returns true when the row existed and was removed, false when absent.
   */
