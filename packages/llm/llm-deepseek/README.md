...
Set `supportsReasoningEffort: false` for a deployment model such as `deepseek-v4-flash-0731` when the endpoint provides DeepSeek Flash without a selectable reasoning control. The model remains selectable, the adapter omits both `thinking` and `reasoning_effort` so the provider keeps its own default behavior, and an explicit reasoning effort fails with `UNSUPPORTED_REASONING_EFFORT` before network I/O.
