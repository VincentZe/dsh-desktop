...
- **语言切换行。** 标准中文文件一律在 H1 标题后立即以 `[English](foo.md) | 中文` 链回英文。普通撰写的英文文件在同一位置以 `English | [中文](foo.zh.md)` 互链；清单内的生成英文源省略此行，以便与生成器输出逐字节一致。根 GitHub 首页文件是明确例外：中文 `README.md` 与 `README.zh.md` 都链接到 `README.en.md`，`README.en.md` 链回 `README.md`。发布到 GitHub 以外位置的 README（例如 PyPI 项目元数据）可以改用指向同一对侧文件的规范 `https://github.com/deepseek-ai/deepseek-harness/blob/master/<repository-path>` URL，使切换行在该位置仍可访问。
...
1. 范围内所有未排除的文档都有完整配对。发现 README 时，basename 不区分大小写，因此 `missions/readme.md` 与其他文档根一样属于范围。
2. 任何已存在的未排除配对产物都完整且一致：三个文件齐全、每一侧的当前 blob hash 等于记录值（改了任一侧而没重新确认配对就变红）、中文侧和所有普通撰写的英文源都带语言切换行（清单内的生成英文源除外）、结构签名按序一致：标题深度、逐字节一致的代码块（信息字符串与内容）、表格行列数、列表类型、有序列表起始编号、列表项数量，以及除切换行之外的每个链接目标。
3. 列为 `excluded` 的文件不参与标准配对。兼容文件只有在根例外中的每条路径都显式列出时才能共存；未列出的 `.zh.md` 与 `.i18n.yaml` 仍会报错。`.agents/notes/archived/` 下冻结的 Agent Note 不受这个持续演进的门禁约束；专用校验器会要求其现有的三个配对文件完整，并将其封存。
...
**范围**：根目录 CONTRIBUTING 文档、除明确排除的 GitHub 首页配对外的全部 README，以及 `.agents/notes/**`、`docs/**` 与 `python/**` 下的全部活跃文档。匹配 README 时只看文件名且不区分大小写，因此今后新增的目录无需再修改 manifest。依赖目录、被忽略的构建产物目录以及冻结的 `.agents/notes/archived/` 目录树只在发现阶段排除，不属于持续演进的翻译源文档。
...
**排除**（不参与标准配对；每条排除路径都必须显式列出）：

- `README.md`、`README.en.md` 与 `README.zh.md`：GitHub 会把根目录的 `README.md` 作为仓库首页；本项目用 `README.md` 与 `README.zh.md` 保存同步的中文内容，并用明确的 `.en.md` 文件名保存英文对侧。
...
**统一要求**：当前及今后纳入范围且未排除的每篇文档，合并时都必须构成完整的双语配对。[scripts/translation-pairing.manifest.json](../../scripts/translation-pairing.manifest.json) 只包含显式排除项；不存在逐文件推进清单或日期分界。
