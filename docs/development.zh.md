...
仓库级的 `pnpm dsh:subagent` 命令通过 dsh SDK JSON-RPC 客户端监督一个 child 任务。它是脚本而不是 Cordis 插件，child 运行时才是插件组合。调用方式、流、交互、会话和退出码约定见[运行 dsh subagent](cookbook/running-subagent.md)。
