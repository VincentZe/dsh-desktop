...
The repository-level `pnpm dsh:subagent` command supervises one child task through the dsh SDK JSON-RPC client. It is a script rather than a Cordis plugin; the child runtime is the plugin composition. See [Run the dsh subagent runner](cookbook/running-subagent.md) for invocation, stream, interaction, session, and exit-code contracts.
